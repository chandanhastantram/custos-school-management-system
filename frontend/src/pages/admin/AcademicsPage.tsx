...
import {
  ...
  Edit, Trash2, Search, Download, BookOpen, Clock, MapPin, User, CheckCircle2, RefreshCw,
} from "lucide-react";
...
import { academicsApi } from "@/services/admin-api";
...
interface ClassData {
  ...
  sections: Section[]; studentCount: number; subjectCount: number;
}
interface Section {
  id: string; name: string; classTeacherId?: string; classTeacherName?: string;
  studentCount: number; room?: string;
}
interface Subject {
  id: string; name: string; code: string;
  type: "core" | "elective" | "lab"; credits: number;
  teacherName?: string; periodsPerWeek: number;
}
...
const AcademicsPage = () => {
  ...
  const { data: classesData, isError: classesError, refetch } = useQuery({
  ...
  useEffect(() => {
    if ((classesError || subjectsError) && !demoMode) setDemoMode(true);
  }, [classesError, subjectsError, demoMode]);

  const classes = demoMode ? DEMO_CLASSES : (classesData?.items || classesData || DEMO_CLASSES);
  const subjects = demoMode ? DEMO_SUBJECTS : (subjectsData?.items || subjectsData || DEMO_SUBJECTS);

  const stats = useMemo(() => ({
    totalClasses: classes.length,
    totalSections: classes.reduce((sum: number, c: ClassData) => sum + c.sections.length, 0),
    totalStudents: classes.reduce((sum: number, c: ClassData) => sum + c.studentCount, 0),
    totalSubjects: subjects.length,
  }), [classes, subjects]);

  const filteredClasses = useMemo(() => classes.filter((c: ClassData) => c.name.toLowerCase().includes(search.toLowerCase())), [search, classes]);
  const filteredSubjects = useMemo(() => subjects.filter((s: Subject) =>
                  ...
                  <div className="flex items-center justify-between w-full pr-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-muted-foreground">{cls.sections.length} sections · {cls.studentCount} students</p>
                      </div>
                    </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                      ...
                      <Card key={section.id}>
                        <CardContent className="p-4">
                          ...
                          <div className="flex items-center gap-4 mt-3 text-xs text-muted-foreground">
                            <span className="flex items-center gap-1"><Users className="h-3 w-3" /> {section.studentCount}</span>
                            {section.room && <span className="flex items-center gap-1"><MapPin className="h-3 w-3" /> Room {section.room}</span>}
                          </div>
                        </CardContent>
                      </Card>
                    ))}
                    <Card className="border-dashed">
                      <CardContent className="p-4 flex items-center justify-center min-h-[100px]">
                        <Button variant="ghost" className="text-muted-foreground"><Plus className="h-4 w-4 mr-1" /> Add Section</Button>
                      </CardContent>
                    </Card>
                  </div>
                </AccordionContent>
            ...
            <Table>
              ...
              <TableBody>
                {filteredSubjects.length === 0 ? (
                  <TableRow><TableCell colSpan={7} className="text-center py-12 text-muted-foreground">No subjects found</TableCell></TableRow>
                ) : (
                  filteredSubjects.map((subject: Subject, i: number) => (
                    <motion.tr key={subject.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                      className="border-b border-border hover:bg-muted/30 transition-colors">
                      <TableCell className="font-medium">{subject.name}</TableCell>
                      <TableCell className="font-mono text-sm">{subject.code}</TableCell>
                      <TableCell><Badge className={cn("gap-1", SUBJECT_TYPE_CONFIG[subject.type].color)}>{SUBJECT_TYPE_CONFIG[subject.type].label}</Badge></TableCell>
                      <TableCell>{subject.credits}</TableCell>
                      <TableCell className="text-sm">{subject.teacherName || "-"}</TableCell>
                      <TableCell><div className="flex items-center gap-1"><Clock className="h-3.5 w-3.5 text-muted-foreground" />{subject.periodsPerWeek}</div></TableCell>
                      <TableCell>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem><Edit className="h-3.5 w-3.5 mr-2" /> Edit</DropdownMenuItem>
                            <DropdownMenuItem><Book className="h-3.5 w-3.5 mr-2" /> Syllabus</DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem className="text-destructive" onClick={() => !demoMode && deleteSubjectMutation.mutate(subject.id)}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {subjects.slice(0, 5).map((subject: Subject) => {
